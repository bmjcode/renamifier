...
Section "Renamifier"
  ...
  Delete "$INSTDIR\libpoppler-130.dll"
  Delete "$INSTDIR\libpoppler-159.dll"
  ...
  File /r styles
  File /r tls
  ; These may or may not be present depending on your system
  File /nonfatal /r translations

  ; Add other library dependencies
  ; To check this: `ldd renamifier.exe | fgrep /mingw | sort`
  File "${MINGW_DIR}\bin\libLerc.dll"
  FILE "${MINGW_DIR}\bin\libb2-1.dll"
  ...
  File "${MINGW_DIR}\bin\libicudt78.dll"
  File "${MINGW_DIR}\bin\libicuin78.dll"
  File "${MINGW_DIR}\bin\libicuuc78.dll"
  ...
  File "${MINGW_DIR}\bin\libnghttp3-9.dll"
  File "${MINGW_DIR}\bin\libngtcp2-16.dll"
  File "${MINGW_DIR}\bin\libngtcp2_crypto_ossl-0.dll"
  ...
  File "${MINGW_DIR}\bin\libpoppler-162.dll"
  File "${MINGW_DIR}\bin\libpoppler-qt6-3.dll"
